...
lose_the_game = true;
  ...
  // for (auto robot: robt_){
  //   if (robot -> inDistress()){
  //     SubjectDistress_ -> ChangeOfState(
  //       new EventDistressCall(robot ->get_id(), robot ->get_pos()));
  //   }
  // }
...
//   CheckForEntityCollision(robt_[3], recharge_station_,
//     &ec, robt_[3]->collision_delta());
// if (ec.happened()) {
// printf("*************************************************************************************************8\n" );
// printf("################################################################ %d\n", robt_[3]->get_id());
// printf("################################################################ %d\n", ec.entity_id1());
  ...
  for (auto ent : mobile_entities_) {
    ...
    if (!ec.collided()) {
      for (size_t i = 0; i < entities_.size(); ++i) {
        ...
        //p = dynamic_cast<Player*>(entities_[i]);
      } /* for(i..) */
    } /* else */
    Robot* r = dynamic_cast<Robot*>(ent);
    if (r) {
...
void Arena::CheckForEntityOutOfBounds( Robot * ent,
  ...
  double an;
  if (ent->get_pos().x+ ent->radius() >=  x_dim_ + ent -> RightProximitySensorRange()) {
    // Right Wall
    event->proximitied(true);
    event->point_of_contact(Position(x_dim_ + ent -> RightProximitySensorRange(), ent->get_pos().y));
    an = ent->heading_angle() - 180;
    event->angle_of_contact(an);
  } else if (ent->get_pos().x- ent->radius()   <= ent -> RightProximitySensorRange()) {
    // Left Wall
    event->proximitied(true);
    event->point_of_contact(Position(ent -> RightProximitySensorRange(), ent->get_pos().y));
    an = ent->heading_angle() - 270;
    event->angle_of_contact(an);
  } else if (ent->get_pos().y+ ent->radius() >= ent -> RightProximitySensorRange() + y_dim_) {
    // Bottom Wall
    event->proximitied(true);
    event->point_of_contact(Position(ent->get_pos().x, ent -> RightProximitySensorRange() + y_dim_));
    event->angle_of_contact(ent->heading_angle());
  } else if (ent->get_pos().y - ent->radius() <= ent -> RightProximitySensorRange()) {
    // Top Wall
    event->proximitied(true);
    event->point_of_contact(Position(ent -> RightProximitySensorRange(), y_dim_));
    event->angle_of_contact(ent->heading_angle());
  } else {
...
void Arena::CheckForEntityCollision(const ArenaEntity* const ent1,
  ...
  double ent2_y = ent2->get_pos().y;
    ...
    //  std::cout<< event -> second_entity_type() << "2^^^^^^^\n";
    //  std::cout<<  ent2 ->entityType() << "2&&&&&&&\n";
    //  std::cout<<  ent1 ->entityType() << "1&&&&&&&\n";


    //  event -> second_entity_type (ent2 ->entityType());
    //  event -> first_entity_type (ent1 -> entityType());
    //  std::cout<<  event->second_entity_type() << "2***********\n";
    //  std::cout<<  event ->first_entity_type() << "1***********\n";
