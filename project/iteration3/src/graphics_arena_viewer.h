 ...
 *
 *  After constructing a new PlayerViewer, call Run() to start and run the
 ...
 *  PlayerViewer app.  Example:
 *
 *  ```
 *  int main(int argc, char **argv) {
 *      cs3081::InitGraphics();
 *      PlayerViewer *app = new PlayerViewer();
 *      app->Run();
...
class GraphicsArenaViewer : public GraphicsApp {
 public:
  ...
  /**
   * @brief Draw the arena with all player, obstacles using nanogui.
   *
...
/**
   * @brief Draw a sensor of robot using nanogui.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
   ...
   */
  void DrawSensor(NVGcontext *ctx, const class Robot* const robot);
  /**
...
/**
   * @brief Draw a homebaseusing nanogui.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
   * @param[in] ctx The nanogui context.
   * @param[in] homebase The homebase handle.
   */
