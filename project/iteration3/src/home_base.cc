...
/* reset the motion handler and the sensor touch and the position*/
void HomeBase::Reset(void) {
  home_base_motion_handler_.Reset();
...
void HomeBase::TimestepUpdate(uint dt) {
  ...
  home_base_motion_handler_.UpdateVelocity(home_base_sensor_touch_);
