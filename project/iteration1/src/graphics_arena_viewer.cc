...
void GraphicsArenaViewer::DrawRobot(NVGcontext *ctx, const Robot* const robot) {
   ...
   * under the robot name
   */
  nvgText(ctx, 0.0, 30.0, robot->str_battery_level().c_str(), NULL);
  /*
   ...
   */
  nvgText(ctx, 0.0, 50.0, robot->loseGame().c_str(), NULL);
...
void GraphicsArenaViewer::DrawHomeBase(NVGcontext *ctx,
 ...
 /*
  * The home base is drawen exactly as the robot is done except the battery
