...
TEST(Robot, Sanity) {
  ...
  EXPECT_EQ(r1.get_pos().x(), 0.0) << "FAIL: Bad X pos";
  EXPECT_EQ(r1.get_pos().y(), 0.0) << "FAIL: Bad Y pos";
  r1.speed(10.0);
...
TEST(Robot, Reset) {
  csci3081::robot_params params;
  params.pos = csci3081::Position(5.0, 5.0);
  params.angle_delta = 15;
  params.battery_max_charge = 100.;
  params.collision_delta = 0.1;
  params.radius = 5.0;
  ...
  EXPECT_EQ(r1.get_pos().x(), 5.0) << "FAIL: Reset Pos X";
  EXPECT_EQ(r1.get_pos().y(), 5.0) << "FAIL: Reset Pos Y";
}
...
TEST(Robot, BatteryDepletion) {
  ...
  // Test that the battery HAS been depleted after that big move.
  EXPECT_TRUE(std::fabs(r1.battery_level() - 100.) <= 0.01) <<
      "FAIL: Battery not depleted";
